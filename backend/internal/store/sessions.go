...
func (s *Store) CreateSession(ctx context.Context, input CreateSessionInput) (*Session, error) {
	...
	err := s.pool.QueryRow(ctx, `
		INSERT INTO sessions (location_id, operator_id, shift_number, plate, city_code, vehicle_type, state, check_in_at)
	...
	`, input.LocationID, input.OperatorID, input.ShiftNumber, input.Plate, input.CityCode, input.VehicleType).Scan(
...
func (s *Store) GetSessionByID(ctx context.Context, id string) (*Session, error) {
	var session Session
	err := s.pool.QueryRow(ctx, `
		SELECT id, location_id, operator_id, shift_number, plate, city_code, vehicle_type, state,
		       check_in_at, check_out_at, fee_amount, rate_snapshot, offline_sync, sync_conflict,
		       created_at, updated_at
		FROM sessions
		WHERE id = $1
